...
- nodes: array of code blocks, each with id, type, label, description, codeRange, variables, assumptions, decision (decision-type only), loop (process with iteration only), parallelGroupId (concurrent nodes only), dependencies
...
- critiques: 1-2 genuinely different structural alternatives (different data structures, control flow, or decomposition). Each should feel actionable — like advice from a code reviewer. The "explanation" field describes how you'd rewrite the code using that approach. The "upside" field should state what you gain in plain language. The "downside" field should state what you give up. Include a short "codeExample" (5-8 lines max, omit if explanation is already clear) showing how the alternative looks in code. The "nodeId" field must be set to the id of the node (from the provided node list) whose code this critique is suggesting a rewrite of.`;
