...
- critiques: exactly 2 alternative structural approaches to the same problem, each with id, title, summary, explanation, tradeoff, and complexity ("simpler"/"similar"/"more complex")
...
11. Generate 1 to 3 concept cards (maximum 3) that capture the most important learning principles
...
15. For critiques, offer genuinely different structural alternatives (e.g. different data structures, different control flow, different decomposition). Each critique should feel actionable — like advice from a code reviewer. The "explanation" field should describe how you'd rewrite the code using that approach. The "tradeoff" field should be honest about both the upside and downside in plain language. Include a "codeExample" showing a short but concrete snippet (5-8 lines maximum, omit if explanation is already clear) of how the alternative approach would look in code.
